...
import 'flashcard.dart'; // Import your necessary files
...
class AddFlashcardScreen extends StatelessWidget {
  ...
  final TextEditingController correctOptionIndexController =
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            buildTextField(
                controller: optionsController, label: 'Options (comma separated)'),
            SizedBox(height: 20.0),
            buildTextField(
  ...
  void saveFlashcard(BuildContext context) {
    ...
    final options =
    optionsController.text.split(',').map((c) => c.trim()).toList();
    final correctOptionIndex =
        int.tryParse(correctOptionIndexController.text) ?? 0;
